        ...
        <Table aria-label="Week class schedule">
            ...
            <TableBody>
                {schedule.map((day,idx)=>(
                    <TableRow key={idx}>
                        <TableCell component="th" scope="row">{dayOfClass+idx}</TableCell>
                        <TableCell>
                            <Typography>{day.topic}</Typography>
                        </TableCell>
                        <TableCell>
                            {day.activity.map(activity=>(
                                <Typography variant="caption" display="block" key={activity}>{activity}</Typography>
                            ))}   
                        </TableCell>
                        <TableCell>
                            <Tooltip title={`Live: ${day.status.live}%, Archived: ${day.status.archived}%`}>
                                <Typography color={parseFloat(day.status.live+day.status.archived)>=75?"primary":"error"}>
                                    {parseFloat(day.status.live+day.status.archived).toFixed(2)}%
                                </Typography>
                            </Tooltip>
                        </TableCell>
                        <TableCell>
    ...
    return <div>
        {data.map((week,idx)=>{
            ...
            return <Accordion
                ...
                className={`${classes.accordion} ${palenl===idx?classes.expandedAccordion:""}`}
                key={idx}
            >
                <AccordionSummary
                    expandIcon={palenl===idx?<RemoveCircleOutlineIcon/>:<AddCircleOutlineIcon/>}
