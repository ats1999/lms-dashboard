...
function getStaus(status) {
    ...
    return [
        ...
        <p className={`${style.status} ${style[`status_${status[2]}`]}}`}>{statusText}</p>,
    ];
}
            ...
            <Table>
                ...
                <TableBody>
                    {questions.map((question, idx) => {
                        const cells = [idx + 1, question.name, ...getStaus(question.status)];
                        return (
...
export default function QuestionTabs({ questions }) {
    ...
    if (!questions || questions.length === 0) return <p>No Questions assigned!</p>;
